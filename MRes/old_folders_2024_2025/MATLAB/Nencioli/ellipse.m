...
% fit_ellipse.m  (call function)  
% fit_ellipse - finds the best fit to an ellipse for the given set of points.  
% Format: ellipse_t = fit_ellipse( x,y,axis_handle )  
% 
% x,y, a set of points in 2 column vectors. AT LEAST 5 points are needed !  
% axis_handle - optional. a handle to an axis, at which the estimated ellipse will be drawn along with it's 
axes 
% Output:    
% ellipse_t - structure that defines the best fit to an ellips e 
...
% short_axis size of the short axis of the ellipse  status       
...
============================================================================
=========  
% We will try to fit the best ellipse to the given measurements. the mathematical  representation of use will 
be the CONIC Equation of the Ellipse which is:  
...
% 
% Cost(A) := (g_c(x_c,y_c;A) -f_c)'*(g_c(x_c,y_c;A) -f_c) = (X*A+f_c)'*(X*A+f_c) = A'*X'*X*A + 
2*f_c'*X*A + N*f^2  
% where:  g_c(x_c,y_c; A) - vector function of ALL the measurements  each element of g_c() is g(x,y;A)  
% X a matrix of the form: [x_c.^2, x_c.*y_c, y_c.^2, x_c, y_c ]  f_c            - is actually defined as 
ones(length(f),1)*f  
...
% NOW, all that is left to do is to extract the parameters f rom the Conic Equation.  
% We will deal the vector A into the variables: (A,B,C,D,E) and assume F = -1; 
% Recall the conic representation of an ellipse:  
% A*x^2 + B*x*y + C*y^2 + D*x + E*y + F = 0  
% We will check if the ellipse has a tilt (=orientation). Th e orientation is present  if the coefficient of the 
term "x*y" is not zero. If so, we first need to remove the  tilt of the ellipse.  
...
% where: c = cos(phi), s = sin(phi)  and simplify...  
...
% Now the constants c=cos(phi)  and s=sin(phi) can be found, and from them  all the other constants 
A`,C`,D`,E` can be found.  
% A` = A*c^2 - B*c*s + C*s^2 D` = D*c -E*s 
% B` = 2*A*c*s +(c^2 -s^2)*B -2*C*c*s = 0 E` = D*s+E*c  
% C` = A*s^2 + B*c*s + C*c^2  
% Next, we want the representation o f the non -tilted ellipse to be as:  
% Ellipse = ( (X -X0)/a )^2 + ( (Y -Y0)/b )^2 = 1  
% where: (X0,Y0) is the center of the ellipse  a,b are the ellipse "radiuses" (or sub -axis)  
...
test = a*c;  
switch (1) 
case (test>0),  status = 1;  
% modified by yuliu  
case (test==0), status = 0; %'Parabola found';  warning( 'fit_ellipse: Did not locate an ellipse' );         % 
modified by yuliu  
case (test<0),  status = 0; %'Hyperbola found'; warning( 'fit_ellipse : Did not locate an ellipse' );         % 
modified by yuliu  
end 
...
if (test>0)  
    ...
    % pack ellipse into a structu re 
    ellipse_t = struct( ... 
        'a',a,... 
        'b',b,... 
        'phi',orientation_rad, ... 
