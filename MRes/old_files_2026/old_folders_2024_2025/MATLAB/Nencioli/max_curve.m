...
% max_curve.m  (call function)  
% max_curve(km_i,km_j,field,km_ci,km_cj,vel) computes the eddy shape  defined as the largest closed 
contour of the streamfunction (PSI) field,  across which velocity magnitude increases.  
% The increase of velocity is checked at the four extremes of the closed  contour (northermost, easternmost, 
southermost and westernmost points).  
% If velocity never increases across a closed contour, the shape isdefined by sim ply the largest closed 
contour around the center.  
...
% km_ci and km_cj are the kilomet ric coordinates of the detected eddy  center;  
% vel is the velocity magnitude field, used to check the increase in  velocity across the closed contour.  
% OUTPUT:  
% eddy_lim is a 2xn array containing the poisition in kilometric coordinates of the n vertices t hat define 
eddy shape; (first row x, second y positions)  
% large is the flag which will be used in 'eddy_dim.m', that indicates if  the shape is normally defined (0), 
or if it's defined as simply  the largest closed contour around the center (1);  
% 'interpolate limits' is used to derive velocity magnitude across the four extremes of a closed contour. 
Check the documentation in 'interpolate limits.m' for further details.  
%-------------------------  
%   Ver. 1.2 May.2010  
%   Ver. 1.1 Dec.2009  
%   Auth ors: Francesco Nencioli, francesco.nencioli@opl.ucsb.edu,  
...
% sort the contours accroding to their maximum latitude; this way t he first  
...
while closed_indx==0 && i<=length(isolines)  
    ...
    if xdata(1)==xdata(end) && ydata(1)==ydata(end) && ... 
        ...
        while  ii<=length(dir) && smaller_vel==0  
            ...
            in_out_vel=interpola te_limits(pts_I(ii),pts_J(ii), ... 
                km_i,km_j,dkm,vel,dir(ii));  
            % change the flag value if velocity decreases  
            if in_out_vel(1,1)>in_out_vel(2,1)  
               if(smaller_vel3==0)  
                 smaller_vel3=1;  
               elseif (smaller_vel2==0)  
                 smaller_vel2=1;  
               elseif (smaller_vel1==0)  
                 smaller_vel1=1;  
               elseif (smaller_vel==0)  
                smaller_vel=1;  
               end 
            end 
...
% eddy_lim is empty if no closed contours of psi exist. In that cas e eddy shape is assumed to be circular 
with radius 'a -1'. See eddy_dim.m for  further details.  
